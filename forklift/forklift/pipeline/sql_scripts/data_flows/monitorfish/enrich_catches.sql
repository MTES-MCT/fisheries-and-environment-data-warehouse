...
SELECT
    c.* EXCEPT (id, vessel_type, main_scip_species_type),
    main_scip_species_type AS trip_main_catch_type,
    COALESCE(s.segment, 'NO_SEGMENT') AS segment,
    landing_datetime_utc,
    port_locode AS landing_port_locode,
    port_name AS landing_port_name,
    port_latitude AS landing_port_latitude,
    port_longitude AS landing_port_longitude
FROM catches_main_type c
LEFT JOIN catches_top_priority_segment s
ON c.id = s.id
LEFT JOIN trips_landing_ports lp
ON
    lp.cfr = c.cfr AND
    lp.trip_number = c.trip_number
...
dep_reports AS (SELECT DISTINCT report_id, operation_number, referenced_report_id, operation_type, flag_state FROM deps),
...
acknowledged_report_ids AS (
   ...
   WHERE
       ...
       AND referenced_report_id IN (
           ...
           UNION ALL
           SELECT operation_number FROM cors_targeting_deps
           UNION ALL
           SELECT operation_number FROM dep_reports
       )
),
